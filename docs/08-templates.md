...
**`TBD`、`TODO`、`未定` を残さない。** `doc_lint` がエラーとして報告する。
...
この2つは**時点の記録**である。あとから書き換えると、なぜその判断をしたのかが失われる。

変わったなら、新しい記録を足す。古い記録には状態（`置き換え済み: ADR-012 を参照`）だけを書き足す（O-02）。
