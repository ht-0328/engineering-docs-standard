...
書籍4冊と、公開されている標準・実験結果6件を突き合わせ、**複数の出典が支持する原則だけ**をルールにした。私の意見だけで書いたルールは入れていない。意見を書いた箇所には「出典なし・私見」と明記している。
...
2. **読者は最初から順に読まない。** 実測で79%が走査し、1語ずつ読むのは16%である（`SRC-EXT-002`）。
...
| C-01 結論を先に書く | 各章の最初に「結論」の節がある |
| C-02 見出しを具体的にする | 「その他」「補足」を使っていない |
| C-08 文書の必須要素 | 冒頭の表が発信日・責任者・タイトルを兼ねている |
| O-01 docs as code | Markdown を正本とし、Git で管理し、`doc_lint` を通している |
| O-06 自動化 | [tools/doc_lint.py](../tools/doc_lint.py) が全ルールのうち機械判定できる分を検査する |
| 出典を示す | すべてのルールに出典IDを付けている。私見には「出典なし・私見」と書いている |
...
| アンチパターン集（07）は自動チェックを大量に抑制している | 悪い例を原文のまま載せる必要があるため。抑制は `<!-- doclint-disable -->` で明示しており、範囲は最小にしている |
