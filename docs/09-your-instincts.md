...
| # | 好み | 判定 | 直接支持した出典数 |
|---|---|---|---|
| 1 | 一文が長いのが嫌い | 支持。ただし理由が違う | 4 |
| 2 | 難しい言葉が嫌い | 支持 | 5 |
| 3 | 横文字（カタカナ語）が嫌い | **部分的に支持。考えを変える必要がある** | 出典間で分かれる |
| 4 | 一つの見出しの下が長いのが嫌い | 支持 | 6 |

**数えたのは「直接支持」だけである。** 方向は同じでも、その原則そのものを述べていない出典は数えていない。区分は [research/cross-reference.md](../research/cross-reference.md) にある。

---

## 1. 一文が長いのが嫌い

この好みは支持される。ただし、問題は本人が思っているところに無い。

### 判定: 支持する。ただし、問題は長さではない

直接支持する出典は `SRC-WRITE-001`、`SRC-WRITE-003`、`SRC-WRITE-004`、`SRC-EXT-001` の4つである。`SRC-EXT-002` は簡潔さ全般を扱うが、一文の長さは扱っていない。
...
4つの好みのなかで、最も多くの出典が直接支持する項目である。

### 判定: 支持する

直接支持する出典は `SRC-WRITE-001`、`SRC-WRITE-003`、`SRC-WRITE-004`、`SRC-EXT-001`、`SRC-EXT-006` の5つである。`SRC-EXT-003`（JTF）は表記の次元で平易さを扱うが、作文技術を目的外と明言しているため、直接支持には数えない。
...
<!-- doclint-disable-next-line vague_words -->
...
直接支持する出典は `SRC-WRITE-002`、`SRC-WRITE-003`、`SRC-WRITE-004`、`SRC-EXT-001`、`SRC-EXT-002`、`SRC-EXT-005` の6つである。**4つの好みのなかで最も多い。**
...
**Webページでは、8割近くが走査していた。** 長い塊は、走査する読者にとって「入口が無い壁」である。**ただしこれはWebページの測定であり、社内文書での比率は分かっていない。**
